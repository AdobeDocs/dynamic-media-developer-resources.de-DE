...
In den aktuellen Versionshinweisen werden die neuesten Funktionen, Verbesserungen und Fehlerbehebungen für [!DNL Adobe Scene7 Publishing System] 6.9 Teil der Adobe Experience Manager-Lösung im [!DNL Adobe Experience Cloud].
...
* ** Erweiterter E-Katalog-Viewer** - Mit einem Inhaltsverzeichnis, Seitenanimation, Imagemap-Überlagerungen und mehr können Sie jetzt ein ansprechenderes Online-Kataloganzeigeerlebnis bereitstellen. Sie können Produkte und Dienste markieren, damit Kunden direkt Links zu Seiten erstellen können, die zu einer höheren Konvertierung führen.
...
* **SSO (Single Sign-On)-Funktion** - Jetzt können Sie Scene7 aus Adobe Experience Cloud nahtlos starten und sich mit Ihrer Adobe ID bei Scene7 anmelden.

   Wenden Sie sich an den technischen Support von Adobe, wenn Sie SSO einrichten möchten, damit Sie sie mit Scene7 verwenden können. Halten Sie die folgenden Informationen bereit, damit Ihre Experience Cloud- und Scene7-Konten verknüpft werden können: Name Ihres Adobe Experience Cloud-Unternehmens; den Namen Ihres Scene7-Unternehmens; und Ihrer geografischen Region.
...
* Videoverschlüsselung ist jetzt standardmäßig verfügbar `Universal_HTML5_MixedMedia_dark` und `Universal_HTML5_MixedMedia_light` Viewer. Siehe [Hinzufügen von Untertiteln zu Video](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/video/adding-captions-video.html#video).
...
* Sichere Tests für Render-Server-Anforderungen - Sichere Tests für Image-Serving-Assets und Video-Assets sind jetzt auch mit Vignetten möglich. Siehe [Testen von Assets vor ihrer Veröffentlichung](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish).
* Überwachung der Cache-Funktion Ungültiges CDN. Nachdem Sie die Vorlage für CDN-Invalidierung ausgewählt haben (siehe [Allgemeine Einstellungen](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html#general-settings)) und eine **[!UICONTROL Ungültiges CDN]** -Anfrage ausgeführt wird, wird nun ein Indikator mit einer Schätzung der Dauer angezeigt, die zum Löschen des Caches erforderlich ist.
...
* Komponenten mit `insertBefore()` API jetzt verwenden `appendChild()` API.
* Hinzugefügte Unterstützung für `setCSS()`, `setModifier()`und `dispose()` -Methoden.
* Hinzugefügte Unterstützung für `getWidth()`und `getHeight()` -Methoden.
* Hinzugefügte Unterstützung für `setPortrait()` und `setLandscape ()` -Methoden.
...
* Hinzugefügt `TableOfContents` -Komponente.
...
* Unterstützung für die Rückgabe von Ausrichtungsspezifikationen hinzugefügt `MediaSetDesc` Objekte.
...
* Korrigiert - Bild wird nach rechts verschoben, wenn `DIV` has `text-align:center` Stil.
...
* Viewer zur Verwendung geändert `Track requests` anstelle von `Page requests` für das Adobe Analytics-Tracking.
...
* Zusätzliche Unterstützung für natives Seiten-Scrollen in HTML5-Viewer-Farbfeldern.
...
* Unterstützung für die Anzeige schmaler Bilder mit `highlightmode` Modifikator für HTML5 Flyout Viewer
* Zusätzliche Unterstützung für die Steuerung der Bildskalierung mithilfe von `zoomfactor` Modifikator zu HTML5 Flyout Viewer.
* Zusätzliche Unterstützung für die Verfolgung von Ereignissen, die vom HTML5 Flyout Viewer generiert wurden.
* Neue Untertitelunterstützung für Videos (nur erstes Video) im HTML5 MixedMedia-Viewer.
* Der anfängliche Bitratenstandard für HTML5-Video- und MixedMedia-Viewer wurde auf 1400 erhöht.
* Unterstützung für Schleifen der Videowiedergabe im HTML5-Video-Viewer hinzugefügt.
* Zusätzliche Unterstützung für Navigationskapitel im HTML5-Video-Viewer
...
* HTML5 MixedMedia-Viewer: Der Viewer wird im responsiven Modus ausgeblendet, wenn das erste Asset ein 2dspinset ist.
...
* Beim Posterbild wird möglicherweise der Fehler &quot;max size&quot;angezeigt. Daher müssen Sie die Limit-Einstellung für die Image Serving-Veröffentlichung erhöhen.
