...
Die neuesten Versionshinweise, in denen die neuesten Funktionen, Verbesserungen und Fehlerbehebungen für [!DNL Adobe Scene7 Publishing System] 6.8 Teil der Adobe Experience Manager-Lösung in [!DNL Adobe Marketing Cloud] beschrieben werden.
...
* Integration von Adobe Analytics-Videoberichten für HTML5-Video-Viewer und Mixed Media Viewer.
* Verfolgung von Ereignissen mehrerer HTML5-Viewer auf derselben Seite.
* Der HTML5-Zoom-Viewer und der HTML5-Viewer für gemischte Medien wurden um `SetIndicator` ergänzt, um Muster auf Mobilgeräten visuell darzustellen.
* Die API `getComponent` wurde hinzugefügt, um externen Zugriff auf HTML5-Viewer-Komponenten zu ermöglichen.
...
* Konvertieren Sie vergessene E-Mail-Formular-HTML in Nur-Text, um auf die Blockierungsliste setz zu vermeiden.
* Die Standardrolle wurde entfernt.
* Neben der Möglichkeit, Inhalte über Server-Verzeichnis-Upload (FTP) in das Scene7 Publishing System hochzuladen und URLs hochzuladen, können Sie jetzt auch Inhalte mit HTTP-POST hochladen. Weitere Informationen zur Verwendung der HTTP-POST finden Sie unter [Image Production System API](/help/aem-ips-api/c-overview.md).
...
* Der HTML5-Video-Viewer verhindert den Zugriff auf das Entsperren des Flash-Plug-Ins.
...
* Es wurde eine Warnmeldung hinzugefügt, wenn Sie die Adobe Analytics-Konfiguration bearbeiten und dann versuchen, ohne zu speichern aus dem Setup zu fliehen.
...
* Flash-Plug-In und Scene7 Publishing System stürzten ab, da eine große Anzahl von Ordnern nicht über ausreichend Flash-Speicher verfügt.
* Wenn Sie den Wert für das benutzerdefinierte Feld &quot;Datum/Uhrzeit&quot;entfernen, sendet das Scene7 Publishing System keine Anforderung an das IPS (Image Publishing System).
...
* Alle Web-Admin-Anforderungen des Scene7 Publishing Systems werden an HTTP weitergeleitet.
...
* Die Firma &quot;label&quot;wird auf der Seite &quot;Adobe Analytics Setup&quot;nicht vollständig angezeigt.
* Metadaten können nicht importiert werden.

## Bekannte Probleme {#section-b2608177928b4174b080d89c938bb0d3}

* Das Update von Adobe Illustrator CC (Creative Cloud) und Adobe InDesign CC vom Mai 2014 wird voraussichtlich Inkompatibilitäten mit Plug-Ins einführen, die Scene7 FXG erstellen. Diese Inkompatibilitäten betreffen die Benutzer der Scene7 Web-to-Print-Lösung. Wenn Sie Scene7 Web-to-Print verwenden, aktualisieren Sie Illustrator und InDesign nicht auf dieses Creative Cloud-Update. Verwenden Sie stattdessen weiterhin die Version vor Mai 2014. Adobe Systems erwartet im Herbst 2014 die Wiederherstellung der Kompatibilität der Scene7 FXG-Erstellung mit neueren Versionen von Illustrator und InDesign.
...
## Funktionen, die nicht mehr unterstützt werden{#section-7b04b6fdb8d1473880ccaf84211254ad}

* Die Silbentrennung wird in der Herbstversion des Scene7 6.9 nicht mehr unterstützt. Die Silbentrennungswörterbücher definieren, wie Wörter aufgeteilt und getrennt werden können, um einen optimalen Textfluss zu unterstützen.
...
* Dynamische (Flash-)Banner werden in dieser Version nicht mehr unterstützt. Die Übernahme dieser Funktion ist gering, da andere Technologien zur Erstellung, Verwaltung und Bereitstellung personalisierter Website-Banner entwickelt wurden. Die Funktion &quot;Scene7 Dynamic Banner (Flash)&quot;funktioniert weiterhin serverseitig, wird jedoch nicht mehr unterstützt.
...
* &quot;Email Communications&quot; und SendMailSimple.jsp Servlet aus Scene7 Publishing System.
* Scene7 FTP-Kontoabschnitt in der Vorlage für Begrüßungs-E-Mail für Media Portal-Benutzer.
...
Siehe [https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/library/viewers-release-notes/archive/c-rn-04-29-14.html](https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/library/viewers-release-notes/archive/c-rn-04-29-14.html)
...
* Korrigiert: `selectSwatch`-Methode, die nicht ausgelöst wird, wenn sie in die Funktion `initViewer()` eingebettet wird.
...
* Unterstützung für den Modifikator `initialBitRate` zur Auswahl der Bitrate für die anfängliche Wiedergabe auf dem Desktop hinzugefügt.
...
* Korrigiert: HTTPS-Protokoll generiert eine Sicherheitswarnung bei IE8 bei der Wiedergabe des Flashs.
* Korrigiert: Die Videogröße wird in Safari 5 im Vollbildmodus bei der Wiedergabe des Flashs nicht korrekt angepasst.
