...
Wiederholbare Texturen können auf flache, fließende Objekte, Skizzen, Ebenen, Wand und Schränke angewendet werden. Wenn das Objekt auf ein nicht texturierbares Objekt angewendet wird, wird es mit `color=` (oder `bgc=` , wenn `color=` nicht angegeben) gemalt.

Ein Material gilt als Textur, wenn es ein `src=` Attribut enthält, das ein Bild angibt, und wenn es in einem MSS auftritt, das kein Dekorrahmen oder Wandrand ist.

Beim Rendern wird die Textur an das Objekt ausgerichtet, indem der `anchor=` Punkt des Texturmaterials mit dem Texturpunkt des Objekts (wie in der Vignette verfasst) übereinstimmt.
