...
Wenn *`thickness`* ein Wert angegeben ist, wird ein Schlagschatten gerendert, wenn das Vignettenobjekt einen entsprechenden Lichtvektor definiert. Auf 0 *`thickness`* einstellen, um das Rendering von Schlagschatten zu deaktivieren.
...
`catalog::Size` wenn das materielle Material auf einem Katalogeintrag basiert; andernfalls `size=0,0,0`. Die Standardgröße wird berechnet, `res=` wenn *`wid`* und *`hei`* sie nicht angegeben ist oder auf 0 eingestellt ist. Es wird kein Schlagschatten gerendert, wenn weder angegeben noch auf 0 gesetzt *`thickness`* ist.
