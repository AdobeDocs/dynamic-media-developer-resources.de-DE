...
Der Texturkolorisierungsalgorithmus von Image Rendering ist ganz einfach - die Komponentenwerte `bgc=` werden von denen der Texturpixel abgezogen, hinzugefügt und schließlich wird das Ergebnis abgeschnitten `color=` und `0,0,0` `255,255,255`.

Bei typischen Verwendungen der Texturfärbung ist der Wert für `bgc=` die wichtigste oder dominierende Farbe im Texturbild. Scene7 Image Authoring bietet halbautomatische Werkzeuge, mit denen Sie vernünftige `bgc=` Farbwerte aus Texturbildern extrahieren können.

Wenn ein Texturmaterial auf ein nicht texturierbares Vignettenobjekt angewendet wird, `bgc=` wird es als Vordergrundfarbe angewendet, wenn `color=` keine Angabe gemacht wird.
...
`catalog::BaseColor` wenn das Material auf einem Katalogeintrag basiert, ansonsten `bgc=808080` (neutral grau).
