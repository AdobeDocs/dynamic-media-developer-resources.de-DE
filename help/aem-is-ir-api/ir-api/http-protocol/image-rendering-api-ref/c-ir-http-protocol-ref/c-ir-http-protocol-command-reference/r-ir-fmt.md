...
In der folgenden Tabelle werden die gültigen Kombinationen aus *`format`* und *`pixelType`*, die entsprechenden HTTP-Antwort-MIME-Typen, ob ICC-Profil eingebettet werden können (siehe [iccEmbed=](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-iccembed.md#reference-47a433138c7c4b29b9b29871b2491a7f)) und welche formatspezifischen Optionsbefehle angewendet werden können.
...
`png-alpha` gibt nicht verknüpftes Alpha zurück (d. h., Alpha multipliziert die Pixelwerte nicht), während `tif-alpha``swf-alpha` und gibt verknüpftes Alpha zurück (d. h. die Alpha-Werte werden vormultipliziert mit den Alpha-Werten). Der Alpha-Kanal entspricht der umgekehrten Vignettenhintergrundmaske für `req=img`die Vignette und bei Bedarf der Gruppen- oder Objektmaske `req=object`. Um Alpha bei Verwendung einer verschachtelten IR-Anforderung anzuwenden, fügen Sie der eingebetteten IR-Anforderung und der Hauptanforderung das entsprechende Alpha-Dateiformat `fmt=` hinzu. Wenn ein CMYK- oder Graustufen-ICC-Profil mit angegeben wird, werden keine Alpha-Daten zurückgegeben `icc=`.
...
*`format`* standardmäßig auf `attribute::Format`und *`tiffCompression`* standardmäßig auf `attribute::TiffEncoding`. *`pixelType`* ist standardmäßig auf `rgb` sofern `icc=` nicht angegeben, andernfalls entspricht es dem Pixeltyp des angegebenen ICC-Profils.
