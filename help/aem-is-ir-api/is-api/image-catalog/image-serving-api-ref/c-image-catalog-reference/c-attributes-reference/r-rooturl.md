...
`attribute::RootUrl` wird anstelle von verwendet,  `attribute::RootPath` wenn ein  `src=` oder - `mask=` Wert durch {geschweifte Klammern} oder (Klammern) eingeschlossen ist.
...
Vererbt von `default::RootUrl` , falls nicht definiert. Wenn definiert, aber leer, werden relative URLs von diesem Bildkatalog nicht unterstützt.
