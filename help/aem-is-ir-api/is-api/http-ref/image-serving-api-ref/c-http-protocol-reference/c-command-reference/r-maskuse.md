...
Die folgende Tabelle zeigt den Effekt `maskUse=` je nach Verfügbarkeit und Typ der Maske (Alpha-Kanal), die mit dem Ebenenbild verknüpft ist.
...
Das Verhalten von `maskUse=` ist nicht definiert und wird nicht unterstützt, wenn es mit Text- oder Farbflächenebenen angegeben wird, wenn keine Bildmaske angewendet wird (mit `mask=` oder `catalog::Mask`).
