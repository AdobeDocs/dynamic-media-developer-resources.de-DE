...
Wenn `attribute::DefaultImageMode=0`dies der Fall ist, *`object`* wird die gesamte ursprüngliche Anforderung ersetzt, auch wenn nur ein Bild in einer Komposition mit mehreren Bildern fehlt. Die einzigen Befehle, die von der ursprünglichen Anforderung beibehalten werden, sind: `wid=`, `hei=`, `fmt=`, `qlt=`.

Wenn `attribute::DefaultImageMode=1`das Objekt nur das fehlende Ebenenbild ersetzt; Attribute für die fehlende Ebene werden angewendet und das Composite wird wie gewohnt verarbeitet und zurückgegeben.

## Eigenschaften {#section-d30923d8dc4042eba10989212dd70887}

Anforderungsattribut. Gilt unabhängig von der aktuellen Ebeneneinstellung. Wird ignoriert, wenn `req=` es sich nicht um `img` oder `tmb`handelt.
...
Image Serving kehrt zurück auf `DefaultImageMode=0` , wenn verschachtelte Image Rendering- oder FXG-Renderanforderungen fehlschlagen.
