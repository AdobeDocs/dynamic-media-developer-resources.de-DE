...
`textPs=` unterstützt erweiterte Funktionen, wie z. B. die Ausrichtung, das Abfließen von Text in nicht rechteckige Bereiche, die mit `textFlowPath=` und/oder `textFlowXPath=`, und das Rendering von Text entlang beliebiger Pfade, die mit `textPath=`.

## Eigenschaften {#section-a289dc26b6534b41998b1e241d5f2f92}

Ebenenattribut. Gilt für `layer=0` if `layer=comp`. Gegenseitig exklusiv mit `src=` und `text=` in derselben Ebene. Das letzte Vorkommen von `text=`, `textPs=`, und `src=` siegt und bestimmt, ob es sich um ein Bild oder eine Textebene handelt. Wird von Effektebenen ignoriert.
