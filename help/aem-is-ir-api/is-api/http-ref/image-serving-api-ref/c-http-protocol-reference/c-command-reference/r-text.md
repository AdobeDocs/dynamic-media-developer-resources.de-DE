...
`text=` unterstützt die automatische Größenanpassung der Textebene, um den gerenderten Text einzupassen (wenn `size=` keine Angabe gemacht wird oder nur die Breite angegeben wird). Beachten Sie, dass in diesem Fall nur einer der RTF-Ausrichtungsbefehle `\ql`, `\qr`und `\qc` angewendet werden kann. Andernfalls wird ein Fehler zurückgegeben.

## Eigenschaften {#section-8c0f020094a44c6b858454ef91ab4edf}

Ebenenattribut. Gilt für `layer=0` if `layer=comp`. Gegenseitig mit `src=` und in derselben Ebene `textPs=` ; das letzte Vorkommen von `text=`, `textPs=`und `src=` siegt und bestimmt, ob es sich um ein Bild oder eine Textebene handelt. Von Effektebenen ignoriert.
