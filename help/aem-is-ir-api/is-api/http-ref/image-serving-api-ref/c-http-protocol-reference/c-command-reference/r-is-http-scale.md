...
No scaling is applied when `scale=1`. *`factor`* kleiner als 1,0 nach unten skaliert und größer als 1,0 vergrößert das Quellbild.

## Eigenschaften {#section-3c7eb45527394fe79b1ddba6c1fcca09}

Quellenbild/Maskenattribut. Wird ignoriert, wenn `size=` dies auch für die aktuelle Ebene angegeben ist. Overrides `res=`. Gilt für Ebene 0, wenn für `layer=comp`angegeben. Wird ignoriert, wenn die Ebene nicht mit einem Bild oder einer Maske verknüpft ist.

## Standard {#section-26e64904362342a5a62c5f6598f330c4}

Wenn nicht angegeben, wird `res=` verwendet. Wenn kein Wert angegeben `res=` ist, wird das Bild ohne Skalierung verwendet.
