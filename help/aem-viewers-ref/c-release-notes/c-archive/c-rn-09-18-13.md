---
description: Verbesserungen, Fehlerkorrekturen und bekannte Probleme in Scene7 Viewers 4.9.2
seo-description: Verbesserungen, Fehlerkorrekturen und bekannte Probleme in Scene7 Viewers 4.9.2
...
Verbesserungen, Fehlerkorrekturen und bekannte Probleme in Scene7 Viewers 4.9.2
...
* Zum Anpassen des QuickInfo-Formats, das Sie der CSS-Deklaration hinzufügen `!IMPORTANT` können.
...
* Die Server-Unterstützung kann keine Viewer-Skins laden, wenn diese für Flash Player 6 kompiliert wurden. Problemumgehung ist die Kompilierung für Flash Player 7.
* Macintosh OS und Flash Player Version 10.0.32.18: Für die Problemumgehung ist eine JavaScript-Brücke anstelle einer LocalConnection erforderlich, um zwischen Flex und Flash zu kommunizieren. Daher muss die Flex-Anwendung in den HTML-Wrapper integriert werden.
* Derzeit unterstützen Flash-Viewer SWFs, die nur für Flash 7 verwendet werden.
...
* Die Änderungen an der `S7Config.setFlashParam()` ermöglichen die Übergabe von mehr als einem Parameter/Wert `syntax S7Config.setFlashParam(<viewer ID>, <parameter name>, <parameter value> [, <parameter name>, <parameter value>])`, in diesem Fall muss jedoch als erster Parameter der Parameter &quot;image&quot; gefolgt vom Parameter &quot;currentFrame&quot; (falls erforderlich) und dann alle anderen Parameter übergeben werden.

* Wenn Sie den `skipFrames=frameList` Parameter zum Auslassen von Bildern verwenden, gibt der Viewer einen Fehler zurück, wenn ein ausgelassener Rahmen direkt von der JavaScript-Funktion zum Ändern des Bildes aufgerufen wird `InitialFrame`, oder bei einem RenderSet, das zwei oder mehr Muster mit einem Bild verknüpft hat - wenn die Musterkombination zum Referenzieren des ausgelassenen Rahmens ausgewählt ist.

* &quot;Beschneiden&quot;wird nicht zusammen mit Zoom-Zielgruppen oder Farbfeldern in den `image=` Argumenten oder `modifier=` Argumenten unterstützt.
...
* Wenn die Viewer mit den neuen Sicherheitsfunktionen von Flash 7 versuchen, von einem anderen Server in derselben Domäne zu lesen, in der sich die Viewer befinden (d. h. von den Pfaden in der `infoServerUrl`, `searchEngineUrl`oder der `serverUrl parameters`), benachrichtigt eine Sicherheitswarnung den Benutzer über diese Tatsache und fragt, ob Sie dies zulassen möchten, es sei denn, die &quot;domänenübergreifende Richtlinie&quot;ist so eingestellt, dass der Zugriff von diesem Server ermöglicht wird.
