...
>[!NOTE]
>
>Diese Schaltfläche unterstützt die Selektoren `state` und `selected` Attribute, die verwendet werden können, um verschiedene Skins auf verschiedene Schaltflächenzustände anzuwenden.
>
>Insbesondere `selected='false'` entspricht dies dem Status der Schaltfläche für den Bildlauf und `selected='true'` entspricht dem Status, in dem das Suchfeld aktiv ist.
