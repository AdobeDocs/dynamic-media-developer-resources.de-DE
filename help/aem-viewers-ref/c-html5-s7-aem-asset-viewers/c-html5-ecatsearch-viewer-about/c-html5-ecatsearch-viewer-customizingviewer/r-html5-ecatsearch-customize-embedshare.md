...
Beispiel - So richten Sie eine Einbettungs-Freigabeschaltfläche ein, die 28 x 28 Pixel groß ist und für jeden der vier verschiedenen Schaltflächenstatus ein anderes Bild anzeigt:
...
>[!NOTE]
>
>Das Kombinationsfeld unterstützt den `expanded` mit möglichen Werten von `true` und `false`. Der Wert `true` wird verwendet, wenn das Kombinationsfeld eine der vordefinierten Einbettungsgrößen anzeigt. Daher sollte er die gesamte verfügbare Breite einnehmen. Der Wert `false` wird verwendet, wenn die Option für benutzerdefinierte Größe im Kombinationsfeld ausgewählt ist. Er sollte daher verkleinert werden, um Platz für benutzerdefinierte Eingabefelder für Breite und Höhe zu bieten.
