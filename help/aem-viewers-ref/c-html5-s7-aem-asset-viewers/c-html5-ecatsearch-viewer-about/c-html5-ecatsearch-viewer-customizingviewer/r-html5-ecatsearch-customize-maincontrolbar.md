...
description: Die Hauptsteuerleiste ist der rechteckige Bereich auf Desktop-Systemen und Tablets, der alle Steuerelemente der Benutzeroberfläche (mit Ausnahme von Schaltflächen für große Seiten) enthält, die für den E-Katalog-Such-Viewer verfügbar sind.
...
>[!NOTE]
>
>Diese Schaltfläche unterstützt die Attributauswahl `state` und `selected`, mit der verschiedene Skins auf verschiedene Schaltflächenzustände angewendet werden können. Insbesondere entspricht `state="selected"` dem anfänglichen Scroll-Button-Zustand, wenn es möglich ist, den Inhalt der Steuerleiste nach links zu scrollen. Die `state="default"` entspricht dem Status, in dem der Inhalt ganz nach links gescrollt wird. Die Bildlaufschaltfläche schlägt vor, den ursprünglichen Status wiederherzustellen.
