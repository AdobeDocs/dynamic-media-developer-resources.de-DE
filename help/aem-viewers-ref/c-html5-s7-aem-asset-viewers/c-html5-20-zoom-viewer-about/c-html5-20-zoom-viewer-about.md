...
<table id="table_ED747CC7178448919C34A4FCD18922D0"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> Blättert durch die Liste der Farbfelder in der Farbfeldleiste. </p> <p> Wenn sich das Bild in einem Reset-Zustand befindet und der <span class="codeph"> Parameter "Frametransition"auf "Slide" </span> festgelegt ist, wird das Asset mit der Diashow-Animation geändert. Bei anderen <span class="codeph"> Frametransition- </span> Modi führt die Geste einen nativen Seitenbildlauf durch. </p> <p> Wenn das Bild vergrößert wird, wird es horizontal verschoben. Wenn das Bild an den Rand der Ansicht verschoben wird und eine Blättergeste in derselben Richtung ausgeführt wird, führt die Geste einen nativen Seitenbildlauf durch. </p> </td> 
  </tr> 
  <tr> 
...
Es wird empfohlen, eine vordefinierte HTML-Seite für den Popup-Modus zu verwenden. Die vordefinierte HTML-Seite wird aufgerufen `ZoomViewer.html` und befindet sich im `html5/` Unterordner Ihrer standardmäßigen IS-Viewer-Bereitstellung wie im Folgenden:
...
Der responsive Design-Einbettungsmodus setzt voraus, dass die Größe des Viewers während der Laufzeit aufgrund der Größenänderung des Containers angepasst werden muss `DIV`. Der häufigste Anwendungsfall ist das Hinzufügen eines Viewers zu einer Webseite mit flexiblem Layout.
...
Wenn die Webseite die Breite und Höhe des Containers des Viewers festlegt `DIV`, füllt der Viewer diesen Bereich und folgt der auf der Webseite angegebenen Größe. Einbetten des Viewers in eine modale Überlagerung, wobei die Größe der Überlagerung der Größe des Webbrowserfensters entspricht.
...
1. Hinzufügen der JavaScript-Datei für den Viewer zur Webseite
   ...
   Zum Erstellen eines Viewers müssen Sie im HTML-Kopf ein Skript-Tag hinzufügen. Bevor Sie die Viewer-API verwenden können, stellen Sie sicher, dass Sie [!DNL ZoomViewer.js]dies berücksichtigen. Die [!DNL ZoomViewer.js] Datei befindet sich im [!DNL html5/js/] Unterordner Ihrer standardmäßigen IS-Viewer-Bereitstellung:
...
1. Einstellen der Viewer-Größe.
   ...
   Im Folgenden sehen Sie ein Beispiel zum Definieren einer statischen äußeren Viewer-Größe in HTML-Seiten:
   ...
   Das Verhalten mit einem festen äußeren Viewer wird im folgenden Beispiel angezeigt. Beachten Sie, dass sich die Größe des äußeren Viewers beim Wechseln zwischen den Sets nicht ändert:
   ...
   Auf der folgenden Demo-Seite wird das Viewer-Verhalten mit einer festen Größe der Haupt-Ansicht dargestellt. Beachten Sie, dass beim Wechseln zwischen Sets die Hauptinhalt-Ansicht statisch bleibt und der Inhalt der Webseite vertikal verschoben wird.
   ...
   Sie können den `stagesize` `params` Modifikator entweder im Viewer-Vorgabendatensatz im Scene7 Publishing System oder explizit mit dem Viewer-Initialisierungscode an die Sammlung oder als API-Aufruf, wie im Abschnitt &quot;Befehlsreferenz&quot;dieser Hilfe beschrieben, wie im Folgenden beschrieben, weiterleiten:
...
1. Erstellen und Initialisieren des Viewers.
   ...
   Wenn Sie die oben genannten Schritte ausgeführt haben, erstellen Sie eine Instanz der `s7viewers.ZoomViewer` Klasse, geben Sie alle Konfigurationsinformationen an den Konstruktor weiter und rufen Sie `init()` die Methode für eine Viewer-Instanz auf.

   Konfigurationsinformationen werden als JSON-Objekt an den Konstruktor übergeben. Dieses Objekt sollte mindestens über ein `containerId` Feld verfügen, das den Namen der Viewer-Container-ID und das verschachtelte `params` JSON-Objekt mit den vom Viewer unterstützten Konfigurationsparametern enthält. In diesem Fall muss für das `params` Objekt mindestens die Image Serving-URL als `serverUrl` -Eigenschaft und das ursprüngliche Asset als `asset` -Parameter übergeben werden. Mit der JSON-basierten Initialisierungs-API können Sie den Viewer mit einer einzelnen Codezeile erstellen und mit einem Beginn versehen.
   ...
   Im Folgenden finden Sie ein Beispiel für das Erstellen einer Viewer-Instanz, das Übergeben der notwendigen Mindestkonfigurationsoptionen an den Konstruktor und das Aufrufen der `init()` Methode. In diesem Beispiel wird davon ausgegangen, dass `zoomViewer` es sich um die Viewer-Instanz, `s7viewer` den Namen des Platzhalter-DIV, die Image-Server-URL und `http://s7d1.scene7.com/is/image/` `Scene7SharedAssets/ImageSet-Views-Sample` das Asset handelt.
...
## Einbetten mithilfe der setter-basierten API {#section-44e014925f24418b900696003855c0a9}
