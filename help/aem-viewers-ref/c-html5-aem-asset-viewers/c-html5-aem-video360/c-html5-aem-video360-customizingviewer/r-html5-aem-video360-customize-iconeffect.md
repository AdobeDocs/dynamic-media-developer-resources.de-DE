...
Symboleffekt unterstützt die `state` Attributauswahl. Der Attributselektor `state="play"` wird verwendet, wenn das Video inmitten der Wiedergabe angehalten wird, und `state="replay"` wird verwendet, wenn sich der Abspielkopf am Ende des Streams befindet.
