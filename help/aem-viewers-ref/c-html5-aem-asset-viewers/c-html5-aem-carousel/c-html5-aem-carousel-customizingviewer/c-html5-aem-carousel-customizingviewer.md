...
description: Die gesamte visuelle Anpassung und die meisten Verhaltensanpassungen für den Karussell-Viewer werden durch die Erstellung eines benutzerdefinierten CSS vorgenommen.
keywords: Responsiv
...
Die gesamte visuelle Anpassung und die meisten Verhaltensanpassungen für den Karussell-Viewer werden durch die Erstellung eines benutzerdefinierten CSS vorgenommen.
...
Der Viewer wird mit vier vorkonfigurierten CSS-Dateien für numerische und gepunktete Anzeigen geliefert, die jeweils in einem „hellen“ und „dunklen“ Farbschema vorliegen. Standardmäßig wird die Version „gepunktetes Licht“ verwendet, aber es ist einfach, durch Verwendung von anderem Standard-CSS und Festlegen des `SetIndicator.mode`-Parameters zu einer anderen Version zu wechseln. Weitere Standard-CSS befinden sich unter dem folgenden Speicherort:
...
Beachten Sie beim Erstellen von benutzerdefiniertem CSS, dass der Viewer `.s7carouselviewer` Klasse seinem Container-DOM-Element zuweist. Wenn Sie eine externe CSS-Datei verwenden, die mit dem `style=`-Befehl übergeben wird, verwenden Sie `.s7carouselviewer` -Klasse als übergeordnete Klasse in einem untergeordneten Selektor für Ihre CSS-Regeln. Wenn Sie der Web-Seite eingebettete Stile hinzufügen, qualifizieren Sie diesen Selektor auch mit einer ID des Container-DOM-Elements wie folgt:
...
Die zweite Gruppe von CSS-Markern enthält `.s7mouseinput` und `.s7touchinput`. Die CSS-`.s7touchinput` wird festgelegt, wenn das aktuelle Gerät eine Touch-Eingabe ist. Andernfalls wird `.s7mouseinput` verwendet. Diese Markierungen sind hauptsächlich dazu gedacht, Benutzeroberflächen-Eingabeelemente mit unterschiedlichen Bildschirmgrößen für verschiedene Eingabetypen zu erstellen, da normalerweise für Touch-Eingaben größere Elemente erforderlich sind.
...
Im Folgenden finden Sie ein „Sprite“-basiertes Beispiel für die Gestaltung desselben Hotspot-Symbols:
